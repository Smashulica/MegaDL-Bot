...
async def help(bot, message, cb=False):
    ...
    me = await bot.get_me()
    button = [[
        InlineKeyboardButton(f'🔙 RESTART', callback_data='back'),
        InlineKeyboardButton(f'DEVELOPER 👨🏻‍💻', callback_data='about')
        ],[
        InlineKeyboardButton(f'🔥 OTRportal ', url='https://t.me/OTRportal'),
...
async def start(bot, message, cb=False):
    ...
    owner_username = owner.username if owner.username else 'Iarbadevanzare'
    button = [[
        InlineKeyboardButton(f'🙌🏻 AJUTOR', callback_data='help'),
        InlineKeyboardButton(f'DEVELOPER 👨🏻‍💻', callback_data="about")
        ],[
        InlineKeyboardButton(f'🔥 OTRportal ', url='https://t.me/OTRportal'),
        InlineKeyboardButton(f'🔐 CLOSE 🔐', callback_data="close")
    ...
    if Config.UPDATES_CHANNEL:
        ...
        try:
            ...
            if user.status == "kicked":
                await message.message.edit(
                    text="Ne sugi pula, ai BAN pe bot. [Grup Support](https://t.me/OTRportal).",
                    parse_mode="markdown",
                    disable_web_page_preview=True
                )
                return
            ...
            await message.message.edit(
                text="**Tot nu ai intrat.. ☹️, Daca doresti sa ma utilizezi intra pe @OTRofficial !**\n\nDin cauza limitei de utilizatori, doar cei care au dat join ma pot utiliza!",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton("🇦🇱 Apasa aici sa intri pe OTRofficial 🇹🇩", url=invite_link.invite_link)
                        ],
                        [
            ...
            await message.message.edit(
                text="Ceva nu e bine. Te rog sa imi dai mesaj [REPORT TO](https://t.me/iarbadevanzare).",
                parse_mode="markdown",
                disable_web_page_preview=True
            )
            return
...
async def cancel_cb(bot, message):
    ...
    try:
        ...
        await message.message.delete()
        await message.message.reply_text("**Process Anulat de User 😡!**", reply_to_message_id=message.message_id)
